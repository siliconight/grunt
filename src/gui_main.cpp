...
int main(int argc, char** argv) {
    ...
    auto do_synth = [&]() -> bool {
        ...
        if (input_mode == 1) {                       // Effort
            ...
            // letters ("aaaargh") slow the delivery so it drawls longer than the
            // short form, preserving the old intensity-from-repetition feel.
            int max_run = 1, run = 1;
            for (size_t i = 1; onomatopoeia[i] && i < sizeof(onomatopoeia); ++i) {
                if (std::tolower((unsigned char)onomatopoeia[i]) ==
                    std::tolower((unsigned char)onomatopoeia[i-1])) {
    ...
    while (!glfwWindowShouldClose(win)) {
        ...
        if (input_mode != 1) {
            ImGui::TextDisabled("(%s mode speaks through Piper — no bank needed)",
                                input_mode == 0 ? "Line" : "Onomatopoeia");
        }
